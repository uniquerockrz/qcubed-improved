<?php
	 ...
	 * @property string $AlertLabelMode
	...
	abstract class QBlockControl extends QControl {
		...
		protected $strAlertLabelMode = null;
		...
		protected function GetControlHtml() {
			...
			switch ($this->strAlertLabelMode) {
				case IAlertLabelMode::Success:
					$this->AddCssClass('alert alert-success');
					break;
				case IAlertLabelMode::Info:
					$this->AddCssClass('alert alert-info');
					break;
				case IAlertLabelMode::Warning:
					$this->AddCssClass('alert alert-warning');
					break;
				case IAlertLabelMode::Danger:
					$this->AddCssClass('alert alert-danger');
					break;
				}
...
//				$strToReturn .= sprintf('<span id="%s_ctldzmask" style="position:absolute;"><span style="font-size: 1px">&nbsp;</span></span>', $this->strControlId);
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case "AlertLabelMode": return $this->strAlertLabelMode;
		...
		public function __set($strName, $mixValue) {
			...
			switch ($strName) {
				...
				case "AlertLabelMode":
					try {
						$this->strAlertLabelMode = QType::Cast($mixValue, QType::String);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
