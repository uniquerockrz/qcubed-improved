<?php
	class QPaginator extends QPaginatorBase {
		...
		protected $strCssClass = 'pagination pagination-sm';
		...
		public function GetControlHtml() {
			...
			$strToReturn .= sprintf('<li class="previous">%s</li>', $strPrevious);
			...
			if ($this->PageCount <= $this->intIndexCount) {
				...
				for ($intIndex = 1; $intIndex <= $this->PageCount; $intIndex++) {
					if ($this->intPageNumber == $intIndex) {
						$strToReturn .= sprintf('<li class="active"><a href="#">%s</a></li>', $intIndex);
					} else {
						$this->strActionParameter = $intIndex;
						$strToReturn .= sprintf('<li><a id="%s" href="%s" %s>%s</a></li>',
			...
			$strToReturn .= sprintf('<li class="next">%s</li>', $strNext);
